...
This document demonstrates how Arbor's shell integration works in practice.
...
When you run `arbor switch --internal my-feature`, the binary outputs:
...
3. Executes `cd /path/to/worktree` when it sees `__ARBOR_CD__`
...
# Now you can use arbor commands:
$ arbor-switch feature-x
__ARBOR_CD__/tmp/example-worktree
...
# Now arbor hook prompt runs before each prompt
...
$ eval "$(arbor init fish --cmd wt --hook prompt)"
```
...
│  Functions:     │
│  - arbor-switch │  Calls: arbor switch --internal
│  - arbor-finish │  Calls: arbor finish --internal
│  - _arbor_exec  │  Parses: __ARBOR_CD__ directives
└────────┬────────┘
         │
         │ arbor switch --internal my-branch
         ▼
┌─────────────────┐
│  Arbor Binary   │  Rust application
│  (Rust)         │
│                 │  Outputs:
│  Commands:      │  - __ARBOR_CD__/path/to/worktree
...
# Expected output:
# Testing arbor shell integration...
#
...
- Directives use `__ARBOR_CD__` prefix (unlikely to conflict with real output)

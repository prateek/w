...
struct TraceEvent {
    ...
    name: String,
    /// Phase: "X" for complete events (with duration)
    ph: &'static str,
    /// Timestamp in microseconds since epoch
    ts: u64,
    /// Duration in microseconds (for "X" phase events)
    dur: u64,
...
pub fn to_chrome_trace(entries: &[TraceEntry]) -> String {
    let trace_events: Vec<TraceEvent> = entries
        ...
        .map(|entry| {
            ...
            let cat = if entry.command.starts_with("git ") {
                Some("git".to_string())
            } else if entry.command.starts_with("gh ") || entry.command.starts_with("glab ") {
...
mod tests {
    ...
    fn test_to_chrome_trace_with_timestamps() {
        let entries = vec![
            make_entry("git status", 10, Some(1000000), Some(1)),
            make_entry("git diff", 20, Some(1000000), Some(2)),
            make_entry("git log", 15, Some(1010000), Some(1)),
        ];
    ...
    fn test_category_assignment() {
        let entries = vec![
            make_entry("git status", 10, Some(0), Some(1)),
            make_entry("gh pr list", 100, Some(0), Some(2)),
            make_entry("glab mr list", 100, Some(0), Some(3)),
            make_entry("echo hello", 1, Some(0), Some(4)),
        ];
    ...
    fn test_args_include_context() {
        let entries = vec![make_entry("git status", 10, Some(0), Some(1))];
